...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:tax="http://www.taxonx.org/schema/v1" xmlns:dc="http://digir.net/schema/conceptual/darwin/2003/1.0" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="paragraph">
		<xsl:choose>
			<xsl:when test="parent::subSubSection[@type = 'document_head']"><xsl:apply-templates/></xsl:when>
			<xsl:when test="parent::subSubSection[@type = 'nomenclature']"><xsl:apply-templates/></xsl:when>
			<xsl:when test="name(child::*) = 'footnote'"><xsl:apply-templates/></xsl:when>
			<xsl:when test="name(child::*) = 'caption'"><xsl:apply-templates/></xsl:when>
			<xsl:otherwise><tax:p><xsl:apply-templates/></tax:p></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	
	<xsl:template match="taxonomicName">
		<xsl:element name="tax:name"><xsl:choose>
				<xsl:when test="@*">
			...
			</xsl:choose><xsl:apply-templates/></xsl:element>
	</xsl:template>
	
	<xsl:template match="materialsCitation"><xsl:if test=".//pageStartToken">
			<xsl:element name="tax:pb">
				<xsl:attribute name="n"><xsl:value-of select=".//pageStartToken/@pageNumber"/></xsl:attribute>
			</xsl:element>
		</xsl:if><xsl:element name="tax:collection_event">
			...
			</xsl:if><xsl:value-of select="normalize-space()"/></xsl:element>
	</xsl:template>
	
	<xsl:template match="//*[ancestor::materialsCitation]"><xsl:choose>
			<xsl:when test="name() = 'pageStartToken'"><xsl:apply-templates select="."/></xsl:when>
			<xsl:when test="./pageStartToken"><xsl:apply-templates select="./pageStartToken"/><xsl:value-of select="normalize-space()"/></xsl:when>
			<xsl:when test="./child::*"><xsl:apply-templates select="./child::*"/><xsl:value-of select="normalize-space()"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="normalize-space()"/></xsl:otherwise>
		</xsl:choose></xsl:template>
	
	<xsl:template match="pageStartToken"><xsl:element name="tax:pb"><xsl:attribute name="n"><xsl:value-of select="./@pageNumber"/></xsl:attribute></xsl:element><xsl:value-of select="normalize-space()"/></xsl:template>
	...
	<xsl:template match="caption">
		<tax:figure>
			<xsl:choose>
				<xsl:when test="./paragraph"><xsl:apply-templates/></xsl:when>
				<xsl:otherwise>
					<tax:p>
						<xsl:apply-templates/>
					</tax:p>
