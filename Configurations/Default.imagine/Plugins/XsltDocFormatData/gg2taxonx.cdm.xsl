...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:tax="http://www.taxonx.org/schema/v1" xmlns:dwc="http://digir.net/schema/conceptual/darwin/2003/1.0" xmlns:dwcranks="http://rs.tdwg.org/UBIF/2006/Schema/1.1" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="mods:mods">
		<mods:mods>
			...
			<mods:identifier type="HTTP-URI"><xsl:choose>
				<xsl:when test="//treatment/@httpUri"><xsl:value-of select="//treatment/@httpUri"/></xsl:when>
				<xsl:otherwise>http://treatment.plazi.org/id/<xsl:value-of select="substring-after(//treatment/@LSID, 'treatment:')"/></xsl:otherwise>
			</xsl:choose></mods:identifier>
		</mods:mods>
		<!--xsl:copy-of select="."/-->
	</xsl:template>
	...
	<xsl:template match="subSection">
		<xsl:choose>
			...
			<xsl:when test="@type = 'reference_group' or @type = 'ref_group'">
				<tax:ref_group>
					<xsl:apply-templates/>
				</tax:ref_group>
			</xsl:when>
	...
	<xsl:template match="taxonomicName">
		<xsl:element name="tax:name">
			...
			<xsl:apply-templates/>
			<xsl:choose>
				<xsl:when test="./ancestor::subSubSection[./@type = 'nomenclature']"></xsl:when>
				<xsl:when test="./following-sibling::text()[1][normalize-space(.) = ',' and name(./following-sibling::*[1]) = 'taxonomicNameLabel']">
					<xsl:text disable-output-escaping="yes">,&#x20;</xsl:text>
					<xsl:element name="tax:namePart">
						<xsl:attribute name="type">status</xsl:attribute>
						<xsl:value-of select="./following-sibling::taxonomicNameLabel"/>
					</xsl:element>
				</xsl:when>
				<xsl:when test="./following-sibling::*[1][name() = 'taxonomicNameLabel']">
					<xsl:text disable-output-escaping="yes">&#x20;</xsl:text>
					<xsl:element name="tax:namePart">
						<xsl:attribute name="type">status</xsl:attribute>
						<xsl:value-of select="./following-sibling::taxonomicNameLabel"/>
	...
	<xsl:template match="taxonomicNameLabel">
		<xsl:choose>
			...
			<xsl:when test="./preceding-sibling::taxonomicName"></xsl:when>
			<xsl:otherwise>
				<xsl:text disable-output-escaping="yes">&#x20;</xsl:text>
